...
function draw() {
  ...
  for (let i = 0; i < allCircles.length; i++) {
    ...
    // Rebound off the top/bottom of the canvas
    if (
    ...
    // Add a random value -2 to 2 to the speed when it rebounds
    let rand = Math.random() * 4 - 2;
    aCircle.ySpeed += rand;
  }

  // Add a random value -5 to 5 to the radius when you press r
  if (rPressed) {
    // change radius
  ...
  // Randomly change the colour of the circle every second

  if (frame == 15) {
    var hexString = "0123456789ABCDEF"; // all hex characters. e.g. #3BA69C
    var hexColour = "#";
    for (let i = 0; i < 6; i++) {
      let randColour = Math.floor(Math.random() * hexString.length);
      hexColour += hexString.charAt(randColour);
    }
